...
void CPU::decode() {
  ...
  switch(opcode) {
    ...
    case 0x04: D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
			   ...
			   pc = pc + (simm << 2);
			   stats.flush(2); stats.countTaken(); 
			   }
			   break; 
    ...
    case 0x2b: D(cout << "sw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               //store word doesn't write to register. notify stats that we are reading regFile pointed out to by rt.
...
void CPU::printFinalStats() {
  ...
  cout << "RAW hazards: " << setprecision(0) << stats.getTotalHazards() << " (1 per every " << fixed << setprecision(2)
       << 1.0 * instructions / stats.getTotalHazards() << " instructions)" << endl;
  cout << "  On EXE1 op: " << stats.getHazards(EXE1) << fixed << setprecision(0) << " (" <<(100.0 * stats.getHazards(EXE1) / stats.getTotalHazards()) << "%)" << endl;
  cout << "  On EXE2 op: " << stats.getHazards(EXE2) << fixed << setprecision(0) << " (" <<(100.0 * stats.getHazards(EXE2) / stats.getTotalHazards()) << "%)" << endl;
  cout << "  On MEM1 op: " << stats.getHazards(MEM1) << fixed << setprecision(0) << " (" <<(100.0 * stats.getHazards(MEM1) / stats.getTotalHazards()) << "%)"<< endl;
  cout << "  On MEM2 op: " << stats.getHazards(MEM2) << fixed << setprecision(0) << " (" <<(100.0 * stats.getHazards(MEM2) / stats.getTotalHazards()) << "%)" << endl;
